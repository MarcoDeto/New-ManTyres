...
export class ModalVeicoliComponent implements OnInit, OnDestroy {
  ...
  protected filtering() {
    ...
    // filtering
    this.filtered.next(
      this.clienti.filter(type => type.nome.toLowerCase().indexOf(search) > -1)
    );
  }
  ...
  submitForm() {
    ...
    this.veicolo = {
      ...
      targa: this.upperCase(this.VeicoloForm.get('targa').value),
      marca: this.replaceSpacesInOne(this.VeicoloForm.get('marca').value),
      modello: this.replaceSpacesInOne(this.VeicoloForm.get('modello').value),
      clienteId: this.VeicoloForm.get('cliente').value,
      isDeleted: false,
      dataCreazione: null,
      cliente: null
    };
